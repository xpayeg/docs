...
If failed, you may receive a response body with the following structure:
...
```json
{
  "status": {
    ...
    "errors": [
      ...
      {"billing_data": {"nested_field": ["error message"]}}
    ]
  },
  "data": {},
  "count": null,
...
#### Example: Payment with Incorrect Billing Info
```json
{
    "status": {
        "code": 400,
        ...
        "errors": [
            {
                "billing_data": {
                    "name": [
                        "Must be 2 words or more. Got 'invalidname'"
                    ],
                    "phone_number": [
...
#### To cancel your recurring payment just head to the transaction tab inside your dashboard and choose Cancel Recurring Payment From the actions



## Payment Options

There are five payment options available at the moment
